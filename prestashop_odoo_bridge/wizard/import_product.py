...
class ImportPrestashopProducts(models.TransientModel):
    ...
    def _get_product_basics(self, prestashop, products):
        ...
        brand_id = products.get("id_manufacturer")
        ...
        if type(products['description']['language'])==list:
            for pro_name in products['description']['language']:
                if pro_name['attrs']['id'] == self.channel_id.ps_language_id:
                    try:
                        website_description = html2text.html2text(pro_name['value'])
                    except:
                        website_description = pro_name['value']
        else:
            try:
                website_description = html2text.html2text(products.get('description')['language']['value'])
            except:
                website_description = products.get('description')['language']['value']
                ...
                try:
                    ...
                    if type(product_data['description_short']['language'])==list:
                        for pro_name in product_data['description_short']['language']:
                            if pro_name['attrs']['id'] == self.channel_id.ps_language_id:
                                try:
                                    vals["website_description"] = html2text.html2text(pro_name['value'])
                                except:
                                    vals['website_description'] = pro_name['value']
                    else:
                        try:
                            vals['website_description'] = html2text.html2text(data.get('description_short')['language']['value'])
                        except:
                            vals['website_description'] = product_data.get('description_short')['language']['value']
    ...
    def _filter_product_using_id(self, prestashop, channel, **kwargs):
        date_created = None
        vals_list = []
        limit = '{},{}'.format(kwargs.get("page"),kwargs.get("page_size"))
        for product_id in [10960,11156,11371,11373,11374,11375,11377,11435,9821,10983,11154,11159,11198,11378]:
    ...
    def import_now(self,**kwargs):
        ...
        if kwargs.get('prestashop_object_id'):
            ...
            data_list = self._filter_product_using_id(
