...
class ProductFeed(models.Model):
    ...
    def update_product_variants(self, variant_ids, template_id, store_id, location_id, channel_id):
        ...
        for variant in variant_objs:
            ...
            exists = self._context.get('variant_mappings').get(channel_id.id, {}).get(store_id, {}).get(
                variant_store_id)
            if not exists:
    ...
    def get_variant_extra_values(self, template_id, variant, channel_id):
        ...
        if variant.website_description:
            vals['website_description'] = variant.website_description
    ...
    def _create_product_line(self, variant, template_id, store_id, location_id, channel_id):
        ...
        if not exists:
            vals = {
                ...
                'website_description': template_id.website_description,
        ...
        if state == 'done':
            if match:
                ...
                if extra_categ_ids:
                    ...
                    if categ_ids:
						#creation in categ_public_ids
                        vals['public_categ_ids']= [(6, 0, categ_ids)]
