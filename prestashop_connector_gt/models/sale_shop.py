	...
	def import_customers(self):
		for shop in self:
			...
			try:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				# prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '>[%s]' % self.last_customer_id_import,'filter[id_shop]': '%s' % self.presta_id, 'limit': 10000}
		...
		try:
			if isinstance(id_country, str):
				...
				if not country_id and id_state == False:
					...
					mm = browse_country_obj.search([('code', '=', country_code)])
					if not mm:
	...
	def import_addresses(self):
		...
		for shop in self:
			try:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				# prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '>[%s]' % self.last_address_id_import, 'limit': 10000}
...
# 				self.env.context = new_context
		...
		try:
			...
			if product_dict.get('ean13') not in ['0','']:
				...
				if not check_product:
					...
					log_id_obj = self.env['prestashop.log'].create(
						{'log_name':'Barcode : '+str(product_dict.get('ean13'))+'Already present','all_operations': 'import_products', 'error_lines': [(0, 0, {'log_description': 'existing in Odoo Internal ref: '+str(check_product.default_code) }),(0, 0, {'log_description': ' New trying create product Internal ref: '+str(check_product.default_code) })]})
	...
	def import_product_inventory(self):
		for shop in self:
			# try:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '>[%s]' % self.last_product_inventory_import, 'filter[id_shop]': '%s' % self.presta_id,'limit': 5000}
				prestashop_stock_data = prestashop.get('stock_availables', options=filters)
				if prestashop_stock_data and 'stock_availables' in prestashop_stock_data and 'stock_available' in prestashop_stock_data.get('stock_availables'):
				...
				if saleorderid.invoice_ids:
					for invoice_id in saleorderid.invoice_ids:
						if invoice_id.state not in ['paid'] and invoice_id.invoice_line_ids:
							payment_register_id = self.env['account.payment.register'].with_context(active_model= 'account.move',active_ids=invoice_id.ids).create({'journal_id': self.sale_journal.id ,'amount':invoice_id.amount_residual})
			...
			for child in order_rows:
				...
				if self.get_value_data(child.get('product_attribute_id')) != '0':
					...
					try:
						...
						temp_id = prod_templ_obj.search([('presta_id', '=',self.get_value_data(child.get('product_id'))),], limit=1)
					except Exception as e:
					...
					if temp_id:
						combination = product_template_attribute_value.search([
							('product_tmpl_id', '=', temp_id.id),
							('product_attribute_value_id', 'in', prod_attr_val_obj.browse(value_list).ids),
							('attribute_id', 'in', prod_attr_val_obj.browse(value_list).mapped('attribute_id').mapped('id'))
						])
						product_id = temp_id._get_variant_for_combination(combination)
						line.update({'product_id': product_id.id, 'product_uom': product_id.uom_id.id})
				else:
					product_id = product_obj.search(
						[('product_tmpl_id.presta_id', '=', self.get_value_data(child.get('product_id'))),], limit=1)
					if product_id:
				...
				if 'product_id' not in line:
					...
					line.update({'product_id': product_id.id, 'product_uom': product_id.uom_id.id})
				if group_tax_id and self.get_value_data(child.get('unit_price_tax_excl')) != self.get_value_data(child.get('unit_price_tax_incl')):
					tax_rule_id = tax_rule_obj.search([('group_id','=',group_tax_id.id)],limit=1)
					line.update({'tax_id': [(6,0,tax_rule_id.tax_id.ids)]})
				line_ids = sale_order_line_obj.search([('presta_id', '=', line.get('presta_id')), ('order_id', '=', orderid.id)],limit=1)
				if not line_ids:
	...
	def create_presta_order(self, order_detail, prestashop):
		...
		try:
			...
			if int(self.get_value_data(order_detail.get('id_carrier'))) > 0:
				delivery_carrier = carrier_obj.search([('presta_id', '=', order_detail.get('id_carrier')), ('is_presta', '=', True)], limit=1)
				delivery_carrier.fixed_price = float(order_detail.get('total_shipping'))
				carrier_obj.set_delivery(sale_order_id,delivery_carrier.id)
	...
	def import_orders(self):
		try:
			for shop in self:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '>[%s]' % shop.last_order_id_id_import,'filter[id_shop]': '%s' % self.presta_id, 'limit': 1}
	...
	def import_messages(self):
		try:
			for shop in self:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				# prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '=[%s]' % shop.last_message_id_import,'filter[id_shop]': '%s' % self.presta_id, 'limit': 20}
	...
	def import_cart_rules(self):
		try:
			...
			for shop in self:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				# prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '=[%s]' % shop.last_cart_rule_id_import,'filter[id_shop]': '%s' % self.presta_id, 'limit': 500}
	...
	def import_catalog_price_rules(self):
		try:
			...
			for shop in self:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				# prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '=[%s]' % shop.last_catalog_rule_id_import,'filter[id_shop]': '%s' % self.presta_id, 'limit': 500}
...
# 		return True
						...
						if product_tmpl_id.presta_id:
							...
							if product_tmpl_id.presta_id:
								if len(product_product_ids) == 1:
									stock_quant_ids = stock_quant_obj.search([('location_id', '=', shop.warehouse_id.lot_stock_id.id),('product_id', '=', product_product_ids.id)])
									quantity_avilable = sum(stock_quant_id.quantity for stock_quant_id in stock_quant_ids)
									shop.update_quantity_prestashop(prestashop, product_tmpl_id.presta_id,quantity_avilable, shop.presta_id)

								else:
									for product_product_id in product_product_ids:
										if product_product_id.combination_id:
											stock_quant_ids = stock_quant_obj.search([('location_id', '=', shop.warehouse_id.lot_stock_id.id),('product_id', '=', product_product_id.id)])
											quantity_avilable = sum(stock_quant_id.quantity for stock_quant_id in stock_quant_ids)
											combination_id = product_product_id.combination_id
											shop.update_quantity_prestashop(prestashop,  product_tmpl_id.presta_id, quantity_avilable,shop.presta_id,combination_id)
		...
		for shop in self:
			try:
				...
				if self.env.context.get('category_ids'):
					...
					category_ids = categ_obj.search([('to_be_exported', '=', True), ('shop_ids', '=', shop.id)])
						...
						if product_tmpl_id.presta_id:
							...
							if product_tmpl_id.presta_id:
								if len(product_product_ids) == 1:
									...
									reserved_quantity = sum(stock_quant_id.reserved_quantity for stock_quant_id in stock_quant_ids)
	...
	# 					'state': {'value': '1'},
					...
					for prod_var in product_var_ids:
						...
						for variants_values in prod_var.product_template_attribute_value_ids:
							...
							value_ids = self.env['product.attribute.value'].search([('name','=',variants_values.name)],limit=1)
	...
	def action_import_taxes(self):
		...
		prestashop = PrestaShopWebServiceDict(self.shop_physical_url,self.prestashop_instance_id.webservice_key or None)
		...
		if type(prestashop_taxes_groups['tax_rule_groups']) == dict:
			if type(prestashop_taxes_groups['tax_rule_groups'].get('tax_rule_group')) == list:
				...
				for taxes_group in list_taxes_group:
					odoo_tax_group_id = account_tax_group_obj.search([('presta_id', '=', taxes_group.get('id'))], limit=1)
					group_vals={'name':taxes_group.get('name'),'presta_id':taxes_group.get('id'),'is_presta':True}
		...
		if type(prestashop_taxes['taxes']) == dict:
			if type(prestashop_taxes['taxes'].get('tax')) == list:
				...
				for taxes in list_taxes:
					odoo_tax_id = account_tax_obj.search([('is_presta','=',True),('presta_id','=',taxes.get('id'))],limit=1)
					tax_type = False
		...
		if type(prestashop_taxes_rule['tax_rules']) == dict:
			if type(prestashop_taxes_rule['tax_rules'].get('tax_rule')) == list:
				...
				for taxes_rule in list_taxes_rule:
					...
					odoo_tax_id = account_tax_obj.search([('is_presta','=',True),('presta_id','=',taxes_rule.get('id_tax'))],limit=1)
					odoo_group_id = account_tax_group_obj.search([('presta_id','=',taxes_rule.get('id_tax_rules_group'))],limit=1)
					rule_vals = {'presta_id': taxes_rule.get('id'),'tax_id':odoo_tax_id.id,'group_id':odoo_group_id.id}
